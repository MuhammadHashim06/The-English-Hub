...
import "./HeroSection.css";
import { Link } from "react-router-dom";
import heroSectionData from "../data/resourceData";

export default function HeroSection() {
  return (
    <div className="resourcehero">
      <div className="rhero">
        <img src={heroSectionData.heroBackground} alt="Hero Background" />
        <div className="rherobox">
          <div className="rherohead">
            <h1>{heroSectionData.heroHeading}</h1>
          </div>
          <div className="rherocontent">
            <p>{heroSectionData.heroContent}</p>
            <Link to={"/signup"}>
              <button>{heroSectionData.heroButton}</button>
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="resourceslist">
        {heroSectionData.resources.map((resource, key) => (
          <div className="resourcecard" key={key}>
            <div className="cardimg">
              <img src={resource.imgurl} alt={`Resource ${key + 1}`} />
            </div>
            <div className="cardbody">
              <h1>{resource.heading}</h1>
              <p>{resource.subheading}</p>
