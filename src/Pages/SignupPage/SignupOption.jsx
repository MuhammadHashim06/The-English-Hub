...
const SignupOptions = () => {
  ...
  const loginWithGoogle = async () => {
    ...
    try {
      ...
      // Call the server-side endpoint to set the custom claim
      await axios.put(`http://localhost:5000/deactivate-user/${uid}`, { uid });

      // Navigate to the next page (signup role selection)
      navigate('/signup/role', { state: { uid } });
  ...
  return (
    <div>
      <div className="signup-box">
        ...
        <div className="signup-buttons">
          <button className="signup-button signup-google" onClick={loginWithGoogle}>
            <i className="fa-brands fa-google"></i>
            Sign up with Google
          </button>

          <button className="signup-button signup-email" onClick={handleEmailSignUp}>
            <i className="fa-solid fa-envelope"></i>
            Sign up with Email
          </button>
        </div>
